...
class ListSeedRegisterPartBalances:
    ...
    def annul(self):
		for seed_register_part_balances in self.list_seed_register_part_balances:
    		seed_register_part_balances.balance = Balance(0)
...
class IterateNode:
	def __init__(self, node, balance)
		self.id = id(self)
		self.node = node
		self.balance = balance
		self.rank = node.rank
	...
	def return_part_total_balance_to(self, register):
		...
		if total_balance > 0:
			rate = register.balance / seed_register.balance
			self
	...
	def __call__(self):
		...
		if not next_iterate_node:
			...
			if self.iterate_node.loto_balance > 0:
				part_of_balance = PartOfBalance(self.root.balance, self.iterate_node.loto_balance)
